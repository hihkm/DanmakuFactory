    ...
    on_package(function (target)
        if is_mode("release") then
            ...
            if is_plat("windows") then
                package_path = path.join(package_dir, package_name .. ".zip")
                os.execv("powershell", {"-Command", string.format("Compress-Archive -Path '%s\\*' -DestinationPath '%s' -Force -CompressionLevel Optimal", tmp_dir, package_path)})
            else
                package_path = path.join(package_dir, package_name .. ".tar.gz")
